...
router.get('/trc721-name/:contractAddress', async (req, res) => {
    try {
      const { contractAddress } = req.params;
  
      // Crea una instancia del contrato TRC721
      const tronWeb = new TronWeb({
        fullHost: 'https://api.trongrid.io/',
        solidityNode: 'https://api.trongrid.io/',
      ...
      // Llama a la función `name` del contrato para obtener el nombre
      const trc721Name = await instance.name().call();
  
      res.json({
        'name': trc721Name,
      });
    } catch (error) {
      console.error('Error al obtener el nombre del contrato TRC721:', error);
      res.status(500).json({ error: 'Error al obtener el nombre del contrato TRC721.' });
    }
  });
